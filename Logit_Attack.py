...
for k in tqdm_notebook(range(0, num_batches)):
    ...
    labels = torch.full((batch_size_cur,), 24).to(device)
    grad_pre = 0
    prev = float('inf')
    ...
    # 生成对抗样本并保存图片
    adv_samples = X_ori + delta.data
    for i in range(batch_size_cur):
        image_id = image_id_list[k * batch_size + i]
        save_path = f"{image_id}_adv.png"
        vutils.save_image(adv_samples[i], save_path, normalize=True)
